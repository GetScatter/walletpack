	...
	static async checkExchanges(){
		...
		for(let i = 0; i < watchers.length; i++){
			...
			if(status !== history.status){
				...
				await StoreService.get().dispatch(Actions.UPDATE_HISTORY, history);

				if(status === 'complete'){
					// TODO: need to solve this with an injected sound service
					//SoundService.ding();

					Framework.pushNotification('Exchange Complete', `Your token exchange has just completed.`);

					watchers = watchers.filter(x => x.id !== history.id);

					const accounts = StoreService.get().state.scatter.keychain.accounts.filter(x => x.sendable() === history.to);
					if(accounts.length){
