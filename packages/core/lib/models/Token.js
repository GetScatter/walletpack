...
export default class Token {
    ...
    constructor(blockchain = null, contract = '', symbol = '', name = null, decimals = null, chainId = ''){
    ...
    network(){
    	...
    	const endorsed = networks.find(x => x.unique() === PluginRepository.plugin(this.blockchain).getEndorsedNetwork().unique());
    	if(!this.chainId || !this.chainId.length) return endorsed;
    	return networks.find(x => x.blockchain === this.blockchain && x.chainId === this.chainId) || endorsed;
    }
	...
	fiatBalance(withSymbol = true, price = null){
		...
		if(StoreService.get().state.prices.hasOwnProperty(this.uniqueWithChain())){
			price = price ? price : parseFloat(StoreService.get().state.prices[this.uniqueWithChain()][StoreService.get().state.scatter.settings.displayCurrency]);
			return `${parseFloat(price * parseFloat(this.amount)).toFixed(4)} ${withSymbol ? StoreService.get().state.scatter.settings.displayCurrency : ''}`;
		}
		else if(this.unusable && StoreService.get().state.prices.hasOwnProperty(unusableReplacement)){
			price = price ? price : parseFloat(StoreService.get().state.prices[unusableReplacement][StoreService.get().state.scatter.settings.displayCurrency]);
			return `${parseFloat(price * parseFloat(this.amount)).toFixed(4)} ${withSymbol ? StoreService.get().state.scatter.settings.displayCurrency : ''}`;
		}
	...
	fiatPrice(withSymbol = true){
		if(StoreService.get().state.prices.hasOwnProperty(this.uniqueWithChain())){
			const price = parseFloat(StoreService.get().state.prices[this.uniqueWithChain()][StoreService.get().state.scatter.settings.displayCurrency]);
			return `${parseFloat(price).toFixed(4)} ${withSymbol ? StoreService.get().state.scatter.settings.displayCurrency : ''}`
		} else {
			return null;
		}
	}
	...
	baseTokenPrice(withSymbol = true){
		if(StoreService.get().state.prices.hasOwnProperty(this.uniqueWithChain())){
			...
			const baseTokenPrice = parseFloat(StoreService.get().state.prices[systemToken.uniqueWithChain()][StoreService.get().state.scatter.settings.displayCurrency]);
			const price = parseFloat(StoreService.get().state.prices[this.uniqueWithChain()][StoreService.get().state.scatter.settings.displayCurrency]);
	...
	symbolClass(){
    	...
    	const icons = ['eth-tusd', 'btc-btc', 'eos-eos', 'eth-dai', 'trx-trx', 'eth-eth', 'fio-fio', 'cyberway'];
	...
	accounts(){
		return StoreService.get().state.scatter.keychain.accounts.filter(x => x.blockchain() === this.blockchain && x.network().chainId === this.chainId).reduce((acc,x) => {
	...
	static sorter(a,b){
		...
		const systemTokenUniques = StoreService.get().state.scatter.networkTokens().map(x => x.uniqueWithChain(false));
